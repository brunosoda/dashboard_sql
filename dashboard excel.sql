...
select
	...
	payments.receita as "receita (k, R$)",
	(payments.paid_count::float / leads.visit_page_count::float) * 100 as "conversao (%)",
	(payments.receita / paid_count / 1000) as "ticket médio (k, R$)"
from leads
left join payments
	on leads.visit_page_month = paid_month


-- (Query 2) Estados que mais venderam no ultimo mes
...
where paid_date between '20210801' and '20210831'
group by state
...
-- (Query 4) Lojas que mais venderam no mes
-- Colunas: loja, vendas (#)
...
where fun.paid_date between '20210801' and '20210831'
group by loja
order by vendas desc
limit 5
...
select
	...
	count(*) as visitas
from sales.funnel
where visit_page_date between '20210801' and '20210831'
